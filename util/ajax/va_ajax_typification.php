...
function va_get_tproblem_details ($db){
    ...
    $type_props = [];
    $comments = $db->get_results($db->prepare('SELECT Id_Kommentar, Kommentar, k.Angelegt_Von, k.Angelegt_Am, lex_unique(Orth, Sprache, Genus) AS morph_Typ, Id_morph_Typ, morph_Typ_Text FROM tprobleme_kommentare k LEFT JOIN morph_Typen USING (Id_morph_Typ) WHERE Id_Problem = %d', $_REQUEST['id_problem']), ARRAY_A);
    foreach ($comments as $ic => $comment){
        ...
        echo '<div style="border: 1px solid #c3c4c7; box-shadow: 0 1px 1px rgba(0,0,0,.04); background: ' . $bgcolor . '; padding: 20px; margin-bottom: 10px;">';
        echo $comment['Kommentar'] . '<br /><br />';
        ...
        echo '<br /><br /><div style="overflow: hidden;"><span style="float: right">' . $comment['Angelegt_Von'] . ' (' . date('d.m.Y H:i', strtotime($comment['Angelegt_Am'])) . ')<span></div>';
        echo '</div>';
    }
