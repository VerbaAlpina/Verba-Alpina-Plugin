        ...
        set_error_handler(function ($severity, $message, $file, $line){
            ...
            switch ($_POST['category']){
                ...
                case 6: //Polygons
                    ...
                    if(isset($_POST['filter']) && isset($_POST['filter']['subElementCategory']) && $_POST['filter']['subElementCategory'] == -1){ //Pseudo category
                        $subCategoryId = 0;
                        foreach ($data as $key => $row){
                            $data[$key][9] = '?' . $subCategoryId++;
                        }
                    }
                    else if(isset($_POST['filter']) && isset($_POST['filter']['subElementCategory']) && $_POST['filter']['subElementCategory'] == -3){ //Tags
                        foreach ($data as $key => $row){
                            $subVal = -1;
                    ...
                    else if(isset($_POST['filter']) && isset($_POST['filter']['subElementCategory']) && $_POST['filter']['subElementCategory'] == -5){ //Dialectometry
                    ...
                    else if (isset($_POST['filter']['subElementCategory']) && isset($_POST['filter']['subElementCategory']) && $_POST['filter']['subElementCategory'] == -4){ //All distinct
                        foreach ($data as $key => $row){
                ...
                case 7: //SQL
                    ...
                    $extra = false;
                    if (isset($_POST['filter']['groupingSQL']) && $_POST['filter']['groupingSQL']){
                        
                        if(strpos($_POST['filter']['groupingSQL'], ';') !== false){
                            return new IM_Error_Result('No semicolons permitted!');
                        }
...
function va_create_result_object ($where_clause, $lang, $epsilon, $grid_cat, IM_Result &$result, &$Ue, &$db, $extra_field = false){
    ...
    else if(isset($_POST['filter']['subElementCategory'])){
    ...
    foreach ($dbresult as $row){
        ...
        $current_array = array($row['Instance']);
        
        $typifications = explode('-+-', $row['Typings']);
        $type_array = array();
        if($typifications[0] !== ''){
            foreach ($typifications as $t){
                ...
                if(!$type_exists){
                    $type_array[] = array ($id, $kind, $type, $source, ($ref? [$ref]: []), $lids);
                }
                ...
                else if (($row['Instance'] == '' || strpos($row['Instance'], '###') === 0) && strpos($row['Instance_Source'], $source) === 0){
                    //An empty record cannot be used, since the tryMerge function of the record window needs information
                    $current_array[0] = $kind . 'TYP' . $type . $row['Instance'];
                }
            }
        }
        ...
        $base_array = array();
        if($row['Base_Types'] != ''){
            $base_types = explode('-+-', $row['Base_Types']);
        ...
        $conceptArray = $row['Concepts']? explode(',', $row['Concepts']): [];
        ...
        if($subElementType === -3){ //Tags
            switch ($_POST['filter']['selectedTag']){
                case 'ERHEBUNG':
                    $va_sub = '#' . substr($row['Instance_Source'], 0, strpos($row['Instance_Source'], '#'));
                    break;
            }
        }
        
        if($subElementType === 'sql'){
            $va_sub = '#' . $row['XXXFIELDXXX'];
        }
        
        $current_array[] = va_create_type_table($type_array, $base_array, $lang, $row['Instance_Source'], $Ue, strpos($row['Instance'], '###') !== false) . implode('', $langData);
		...
		//Meanings
		$meaningArray = $row['Meanings']? explode('-+-', $row['Meanings']): [];
		$lang = false;
		$meanings = [];
		foreach ($meaningArray as $mdata){
			list($lang, $meaning) = explode(':', $mdata);
			$meanings[] = $meaning;
		}
		if ($lang == 'D'){
			$lang = 'deu';
		...
		$current_array[] = [$lang, $meanings];
		
        
        $current_array[] = $row['Instance_Original'];
        
        $current_array[] = $row['Instance_Encoding'];
        
        $current_array[] = $row['Geonames_Id'];
        
        $current_array[] = $row['Id_Community'];
        
        $current_array[] = va_community_download_link($row['Id_Community']);
        
        $current_array[] = $row['External_Id']; //external_id
            ...
            switch ($_POST['filter']['markings']['tagName']){
                case 'ERHEBUNG':
                    foreach ($_POST['filter']['markings']['tagValues'] as $key => $color){
                        if(strtolower($key) == strtolower(substr($row['Instance_Source'], 0, strpos($row['Instance_Source'], '#')))){
                            $markingColor = $color;
                            break;
                        }
                    }
        ...
        $map_data[] = [$va_sub, new IM_RecordInfoWindowData($current_array), $row['Geo_Data'], $row['Id_Informant'] /* id_informant as stub for quantify data */, $row['Cluster_Id'], $markingColor];
    }
...
function va_get_db_results ($where_clause, $epsilon, $grid_cat, &$db, $extra_field){
    ...
    $query = "SELECT
				...
				GROUP_CONCAT(DISTINCT Meaning SEPARATOR '-+-') AS Meanings,
				Instance_Source,
				" . $geo_sql . " AS Geo_Data,
				Community_Name,
				Id_Informant,
				...
				Cluster_Id,
				Id_Stimulus,
				(SELECT Stimulus FROM Stimuli s WHERE s.Id_Stimulus = z.Id_Stimulus) AS Stimulus,
				Geonames_Id,
                Id_Community,
        ...
        if($count_rest > 0){
            ...
            if($count_rest == 1){
                ...
                if($indexes[0] == $va_index){
                    ...
                    $tname = va_add_references('<span class="va_lex_type" data-id="' . $types[$va_index][0] . '" data-lids="' . $types[$va_index][5] . '">' . $tname . '</span>' . $tlang, $types[$va_index][4]);
                }
                $result .= '<td>' . $tname . '</td><td>' . $types[$indexes[0]][3] . '</td>';
            }
            else {
                if ($va_index !== false){
                    $type_name_td =  va_add_references('<span class="va_lex_type" data-id="' . $types[$va_index][0] . '" data-lids="' . $types[$va_index][5] . '">' . $types[$va_index][2] . '</span>', $types[$va_index][4]);
                }
                else {
...
function va_community_download_link ($id){
    ...
    if ($records_exist){
        ...
        return '<br /><br /><a href="' . $base_link . '" target="_BLANK" style="margin: 5px;"><i class="list_export_icon fas fa-file-download" title="' . $Ue['API_LINK'] .'"></i></a>';
    }
    return '';
}
